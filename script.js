...
window.onload = () => {
  const heroBackgroundImages = document.querySelectorAll(
    "#hero .background img"
  );
  ...
  function SendAppointment() {
    ...
    hoursLis.forEach((hourLi) => {
      hourLi.onclick = () => {
        if (
          hourLi.textContent != "12:00 PM" ||
          hourLi.textContent != "1:00 PM"
        ) {
  ...
  function addToCard() {
    const btnsAddProduct = document.querySelectorAll(
      ...
      btnAddProduct.onclick = () => {
        ...
        setTimeout(() => {
          if (window.localStorage.getItem(`${prodID}`)) {
            let newCount =
              parseInt(window.localStorage.getItem(`${prodID}`)) + 1;
